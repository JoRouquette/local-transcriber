...
public sealed class TranscriptTools
{
    ...
    public string ListTranscripts(
        ...
        => Json(_index.ListDocuments(project, limit).Select(d => new
        {
            d.Project, d.BaseName, d.Language, d.DurationSeconds, d.Speakers, d.TranscribedAt, d.Path
        }));
    ...
    [Description("Recherche dans les transcriptions. mode = hybrid (defaut, semantique + mots-cles), semantic, ou keyword. Filtres optionnels par projet et locuteur.")]
        ...
        CancellationToken ct = default)
    {
        ...
        return Json(results.Select(r => new { r.Project, r.BaseName, r.Speakers, r.Snippet, r.Score, r.Mode, r.Path }));
    }

    [McpServerTool(Name = "get_transcript")]
    [Description("Retourne une transcription par tours de parole, paginee. Filtre optionnel par locuteur. Utilise next_offset pour la page suivante.")]
    public string GetTranscript(
        [Description("Chemin (.json ou .md) retourne par les autres outils.")] string path,
        [Description("Ne renvoyer que les tours de ce locuteur (optionnel).")] string? speaker = null,
        ...
        [Description("Nombre de tours par page (defaut 50).")] int limit = 50)
    {
        ...
        if (resolved is null || !File.Exists(resolved))
            return Json(new { error = "Transcription introuvable ou hors du dossier de sortie.", path });

        var turns = ToTurns(TranscriptReader.ReadSegments(resolved));
        if (!string.IsNullOrWhiteSpace(speaker))
            turns = turns.Where(t => string.Equals(t.Speaker, speaker, StringComparison.OrdinalIgnoreCase)).ToList();
        ...
        return Json(new
        {
            ...
            offset,
            next_offset = next,
            turns = page.Select(t => new { t.Speaker, start = Math.Round(t.Start, 2), t.Text }),
        });
    }
